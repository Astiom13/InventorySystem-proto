...
private:
	...
	UUserWidget* GitHiddenCursorWidget();
	bool IsSameStackable(UInv_InventoryItem* ClickedInventoryItem) const;
	void SwapWithHoverItem(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex);
	bool ShouldSwapStackCounts(const int32 RoomInClickedSlot, const int32 HoveStackCount, const int32 MaxStackSize) const;
	...
	bool ShouldFiilInStack(const int32 RoomInClickedSlot, const int32 HoveredStackCount) const;
	...
	UFUNCTION()
	void OnvInventoryMenuToggled(bool bOpen);
	...
	// index where item would be placed on the grid at a valid location.
