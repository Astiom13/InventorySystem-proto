...
void UInv_InventoryComponent::TryAddItem(UInv_ItemComponent* ItemComponent)
{
	...
	{
		// add stacks to an item already exists in inventory. We only want to update the stack count, not create a new item of this type.
		OnStackChange.Broadcast(Result);
		Server_AddStacksToItem(ItemComponent, Result.TotalRoomToFill, Result.Remainder);
	}
	else if (Result.TotalRoomToFill > 0)
	{
		// this item type doesn't exist in inventory. Create a new one and update all pertinent slot.
...
void UInv_InventoryComponent::SpawnDroppedItem(UInv_InventoryItem* Item, int32 StackCount)
{
	...
	// the item manifest spawns the actor;
...
void UInv_InventoryComponent::Multicast_EquipSlotClicked_Implementation(UInv_InventoryItem* ItemToEquip,
	UInv_InventoryItem* ItemToUnEquip)
{
	// Equipment component will listen to this delegate
