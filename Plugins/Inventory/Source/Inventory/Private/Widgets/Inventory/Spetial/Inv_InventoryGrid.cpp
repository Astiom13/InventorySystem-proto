  ...
  void UInv_InventoryGrid::NativeOnInitialized()
{
	...
	InventoryComponent->OnStackChange.AddDynamic(this, &ThisClass::AddStacks);
  	InventoryComponent->OnInventoryMenuToggled.AddDynamic(this, &ThisClass::OnvInventoryMenuToggled);
}
...
{
  	// if mouse not in canvas panel returns
  ...
  EInv_TileQuadrant UInv_InventoryGrid::CalculateTileQuadrant(const FVector2D& CanvasPosition,
	  const FVector2D& MousePosition) const
  {
  	// Calculate relative position in the current tile
  	const float TileLocalX = FMath::Fmod(MousePosition.X - CanvasPosition.X, TileSize);
  	const float TileLocalY = FMath::Fmod(MousePosition.Y - CanvasPosition.Y, TileSize);

  	// Determine which quadrant the mouse is in
  	const bool bIsTop = TileLocalY < TileSize / 2.f; // Top is Y in the upper half
  ...
  void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
  {
	...
	const FIntPoint StartingCoordinates = CalculateStartingCoordinate(Parameters.TileCoordinates, Dimensions, Parameters.TileQuadrant);
	  ...
	  const FIntPoint& Dimensions)
  {
  	...
  	FInv_SpaceQueryResult Result;
  		...
  		// are the dimensions in grid bounce?
  	if (!IsInGridBounds(UInv_WidgetUtils::GetIndexFromPosition(Position,Columns), Dimensions)) return Result;

  	Result.bHasSpace = true;
  	
  		// if more than one of the indices is occupied with the same item, wt need to see if they all have the same upper-left index.
  	TSet<int32> OccupiedUpperLeftIndices;
	UInv_InventoryStatics::FotEach2D(GridSlots, UInv_WidgetUtils::GetIndexFromPosition(Position,Columns), Dimensions, Columns, [&](const UInv_GridSlot* GridSlot)
	{
		if (GridSlot->GetInventoryItem().IsValid())
		{
  		...
  		// if so, is the only one item in the way(can we swap?)?
  	if (OccupiedUpperLeftIndices.Num() == 1) // single item at position - it is valid for swaping/combining
  	{
	  ...
	  const FVector2D& Location)
  {
  	...
  	bMouseWithinCanvas = UInv_WidgetUtils::IsWithinBounce(BoundaryPos, BoundarySize, Location);
  ...
  void UInv_InventoryGrid::HighlightSlots(const int32 Index, const FIntPoint& Dimensions)
  {
  	if (!bMouseWithinCanvas) return;
	UnHighlightSlots(LastHighlightedIndex, LastHighlightedDimensions);
  	UInv_InventoryStatics::FotEach2D(GridSlots, Index, Dimensions, Columns, [&](UInv_GridSlot* GridSlot)
  	{
  ...
  {
  	UnHighlightSlots(LastHighlightedIndex, LastHighlightedDimensions);
  	UInv_InventoryStatics::FotEach2D(GridSlots, Index, Dimensions, Columns, [State = GridSlotState](UInv_GridSlot* GridSlot)
  	{
	    switch (State)
	    {
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const UInv_InventoryItem* Item, const int32 StackAmountOverride)
{
  ...
  FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
  {
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		// Can the item fit her? (i.e. is it out of the grid bound
	...
	const TSet<int32>& CheckedIndices, TSet<int32>& OutTentativelyClaimed, const FGameplayTag& ItemType, const int32 MaxStackSize)
{
	...
	UInv_InventoryStatics::FotEach2D(GridSlots, GridSlot->GetIndex(), Dimensions, Columns, [&](const UInv_GridSlot* SubGridSlot)
	{
  ...
  bool UInv_InventoryGrid::CheckSlotConstrains(const UInv_GridSlot* GridSlot, const UInv_GridSlot* SubGridSlot, const TSet<int32>& CheckedIndices,
...
FIntPoint UInv_InventoryGrid::GetItemDimensions(const FInv_ItemManifest& Manifest) const
{
  ...
  void UInv_InventoryGrid::RemoveItemFromGrid(UInv_InventoryItem* InventoryItem, const int32 GridIndex)
  {
  	...
  	UInv_InventoryStatics::FotEach2D(GridSlots, GridIndex, GridFragment->GetGridSize(), Columns, [&](UInv_GridSlot* GridSlot)
  	{
  ...
  void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
  {
  	...
  	if(IsSameStackable(ClickedInventoryItem))
  	{
  		...
  		// Should we swap their stack counts? (Room in the clicked slot == 0 && Hovered stack count < MaxStackSize)
  		...
  		// Should we consume the hover item stacks? (Room in the clicked slot >= HoveredStackCount)
  		if (ShouldConsumeHoverItemStacks(HoveredStackCount, RoomInClickedSlot))
  		{
  			// Consume hover item stacks
  		...
  		if(ShouldFiilInStack(RoomInClickedSlot, HoveredStackCount))
  		{
  		...
  		// Make sure we can swap with the valid item
...
{
	// Get Grid Fragment so we know how many grid spaces the item need.
  ...
  UUserWidget* UInv_InventoryGrid::GetVisibleCursorWidget()
{
  ...
  UUserWidget* UInv_InventoryGrid::GitHiddenCursorWidget()
{
  	if (!IsValid(GetOwningPlayer())) return nullptr;
  ...
  bool UInv_InventoryGrid::IsSameStackable(UInv_InventoryItem* ClickedInventoryItem) const
  {
  ...
  bool UInv_InventoryGrid::ShouldSwapStackCounts(const int32 RoomInClickedSlot, const int32 HoveStackCount,
	  const int32 MaxStackSize) const
  {
  	return RoomInClickedSlot == 0 && HoveStackCount < MaxStackSize;	
  }
	  ...
	  const int32 Index)
{
  ...
  bool UInv_InventoryGrid::ShouldFiilInStack(const int32 RoomInClickedSlot, const int32 HoveredStackCount) const
{
	return  RoomInClickedSlot < HoveredStackCount; 	
}
...
{
  	if (!IsValid(GetOwningPlayer())) return;
  	GetOwningPlayer()->SetMouseCursorWidget(EMouseCursor::Default, GitHiddenCursorWidget());
}
...
void UInv_InventoryGrid::DropItem()
{
  ...
  bool UInv_InventoryGrid::HasHoveredItem() const
  {
  ...
  void UInv_InventoryGrid::OnPopUpMenuConsume(int32 Index)
{
  ...
  void UInv_InventoryGrid::OnvInventoryMenuToggled(bool bOpen)
  {
  	if (!bOpen)
  	{
  		PutHoverItemBack();
