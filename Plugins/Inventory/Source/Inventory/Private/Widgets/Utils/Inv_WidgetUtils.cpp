...
bool UInv_WidgetUtils::IsWithinBounce(const FVector2D& BoundaryPos, const FVector2D& WidgetSize,
	...
	const FVector2D& MousePos)
{
	...
	if (MousePos.X + WidgetSize.X > Boundary.X) // Widgets exceeds the right edge
	{
		ClampedPosition.X = Boundary.X - WidgetSize.X;
	}
	if (MousePos.X < 0) // Widgets exceeds the left edge
	{
		ClampedPosition.X = 0;
	}

	// Adjust vertical position to ensure that the widget stays within the boundary
	if (MousePos.Y + WidgetSize.Y > Boundary.Y) // Widgets exceeds the bottom edge
	{
		ClampedPosition.Y = Boundary.Y - WidgetSize.Y;
	}
	if (MousePos.Y < 0) // Widgets exceeds the top edge
	{
		ClampedPosition.Y = 0;
	}
