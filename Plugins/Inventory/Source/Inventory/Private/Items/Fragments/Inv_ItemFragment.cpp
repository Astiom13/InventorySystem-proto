...
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Health Potion consumed: %f."), GetValue()));	
}

void FInv_ManaPotionFragment::OnConsume(APlayerController* PC)
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, FString::Printf(TEXT("Mana Potion consumed: %f."), GetValue()));	
}

void FInv_StrengthModifier::OnEquip(APlayerController* PC)
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Strength increased by: %f."), GetValue()));
}

void FInv_StrengthModifier::OnUnequip(APlayerController* PC)
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Item unequipped.Strength decreased by: %f."), GetValue()));
}
